(function() {
    ...
    if (window.File && window.FileReader && window.FileList) {
        ...
        function generate(gif) {
            ...
            var hMargin = $('#margin').val() === 'left' || $('#margin').val() === 'right' ? margin : 0;
            var vMargin = $('#margin').val() === 'top' || $('#margin').val() === 'bottom' ? margin : 0;
            var minWidth =  0.78 * dpi + hMargin;
            var maxColumn = Math.floor(pageWidth / minWidth);
            var columnNum = Math.min(maxColumn, Math.max(1, parseInt($('#column_num').val(), 10)));
            var cellWidth = Math.floor(pageWidth / columnNum);
            var repeatNum = Math.min(100, Math.max(1, parseInt($('#repeat_num').val(), 10)));
            var rowNum = Math.floor((gif.get_length() * repeatNum + columnNum - 1) / columnNum);
            var pageCanvas, pageContext;
            var imageWidth = cellWidth - hMargin;
            var imageHeight = gif.get_canvas().height * imageWidth / gif.get_canvas().width;
                    ...
                    for (k = 0; k < columnNum; k++) {
                        if (frame < gif.get_length()) {
                            ...
                            pageContext.drawImage(
                                    gif.get_canvas(),
                                    k * cellWidth + leftMargin, j * cellHeight + topMargin,
                                    imageWidth, imageHeight);
                            pageContext.fillText(frame + 1, k * cellWidth + 10, j * cellHeight + 22);
                            frame++;
                            if (frame == gif.get_length()) {
                                repeat++;
                                if (repeat < repeatNum) {
